...
import Selector from "@/components/pong/Selector";
import SideSelector from "@/components/pong/SideSelector";
import Slider from "@/components/pong/Slider";
...
export default function Flip_Card({Lobby, text, title, img, type}: Props) {
    ...
    return (
        <main className={styles.flip_card}>
            ...
            <div className={`${styles.inner} ${type === "rank" && inMatchMaking && styles.turned}`}>
                ...
                <div className={styles.front}>
                    <button className={styles.button}>
                        <div className={styles.button_icone}>
                            <Image src={`/images/${img}.png`} alt={text} width="120" height="120"/>
                        </div>
                        <h1>{title}</h1>
                        <p>{text}</p>
                    </button>
                ...
                {/* Back */}
                {/* Create back */}
                { type == "create" && <div className={styles.back}>
                    ...
                    <div className={styles.settings}>
                        <form>
                            ...
                            {/* Name */}
                            <label>Name</label>
                            <input className={styles.name_input} type="text" id="name" name="name" value={name} onChange={(event) => setName(event.target.value)} />
                            ...
                            <Slider min={1} max={5} step={1} value={difficulty} setValue={setDifficulty} />
                            ...
                            <ImgSelector id="side" value={background} setValue={setBackground} imgs={["background/0","background/1","background/2","background/3"]}/>

                            {/* Ball */}
                            <label>Ball</label>
                            <ImgSelector id="side" value={ball} setValue={setBall} imgs={["ball/0","ball/1","ball/2","ball/3"]}/>

                            {/* Paddle */}
                            <label>Paddle</label>
                            <ImgSelector id="side" value={paddle} setValue={setPaddle} imgs={["paddle/0","paddle/1","paddle/2","paddle/3"]}/>

                            {/* Validate */}
                            <button className={styles.validate_button} type="button" onClick={Create_Game}> 
                                Create game
                            </button>
                    </form>
                    </div>
                ...
                {/* Create back */}
                { type == "training" && <div className={styles.back}>
                ...
                {/* Rank back */}
                { type == "rank" && <div className={styles.back}>

                    {inMatchMaking &&<Image src={`/images/game/balls.gif`} alt="Searching giff" width="120" height="120"/>}

                    {inMatchMaking && <button className={styles.button_back_card} onClick={Stop_Matchmake}>
                        <p>Stop searching</p>
                    </button>}

                    {!inMatchMaking && <button className={styles.button_back_card} onClick={Start_Matchmake}>
                        <p>Start searching</p>
                    </button>}
