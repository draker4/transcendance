...
export default function ConfirmEmailCode() {
	...
	const	inputs = indexes.map((index) => {
		...
		<React.Fragment key={index}>
			...
			<input
				...
				onInput={(event) => moveToNextInput(index, (event.target as HTMLInputElement).value)}
	...
	const	onSubmit = async (data: FormInputs) => {
		...
		try {
			const	response = await fetch(`http://localhost:3000/api/auth/verify?code=${code}`);
	...
	return (
		<div className={styles.main}>
			<h3 className={styles.title}>Please enter the verification code sent to your email address!</h3>
